...
from imagegenerator import create_aggregate_image, map_sections, cache_directory, parse_file
...
@init.bot.tree.command(name="loadconfig", description="Select and load a maps config file")
async def loadconfig(ctx: discord.Interaction):
    if "Admin" in [role.name for role in ctx.user.roles]:
        config_files = get_config_files()
        if not config_files:
            await ctx.response.send_message("No configuration files available.", ephemeral=True)
            return
        view = ConfigSelectView(config_files)
        await ctx.response.send_message("Select a configuration file to load:", view=view, ephemeral=True)
    else:
        await ctx.response.send_message("You do not have the required permissions to perform this action.", ephemeral=True)

@init.bot.tree.command(name="add_category", description="Add a new category to the configuration")
async def add_category(ctx: discord.Interaction, category_name: str):
    if "Admin" in [role.name for role in ctx.user.roles]:
        ...
        if category_name in init.MAPS.sections():
            await ctx.response.send_message(f"Category '{category_name}' already exists.", ephemeral=True)
        else:
            init.MAPS.add_section(category_name)
            with open(init.config_file, 'w') as configfile:
                init.MAPS.write(configfile)
            await ctx.response.send_message(f"Category '{category_name}' added successfully.", ephemeral=True)
    else:
        await ctx.response.send_message("You do not have the required permissions to perform this action.", ephemeral=True)

@init.bot.tree.command(name="add_map", description="Add or update a map in a category")
async def add_map(ctx: discord.Interaction, category_name: str, map_name: str, map_id: str):
    if "Admin" in [role.name for role in ctx.user.roles]:
        init.MAPS.read(init.config_file)
        ...
        with open(init.config_file, 'w') as configfile:
            init.MAPS.write(configfile)
        await ctx.response.send_message(f"Map '{map_name}' with ID '{map_id}' added/updated in category '{category_name}'.", ephemeral=True)
    else:
        await ctx.response.send_message("You do not have the required permissions to perform this action.", ephemeral=True)

@init.bot.tree.command(name="remove_category", description="Remove a category from the configuration")
async def remove_category(ctx: discord.Interaction, category_name: str):
    if "Admin" in [role.name for role in ctx.user.roles]:
        ...
        if category_name in init.MAPS.sections():
            ...
            with open(init.config_file, 'w') as configfile:
                init.MAPS.write(configfile)
            await ctx.response.send_message(f"Category '{category_name}' removed successfully.", ephemeral=True)
        else:
            await ctx.response.send_message(f"Category '{category_name}' does not exist.", ephemeral=True)
    else:
        await ctx.response.send_message("You do not have the required permissions to perform this action.", ephemeral=True)
    ...
    if "Admin" in [role.name for role in ctx.user.roles]:
        ...
        if category_name in init.MAPS.sections():
            if map_name in init.MAPS.options(category_name):
                ...
                with open(init.config_file, 'w') as configfile:
                    init.MAPS.write(configfile)
                await ctx.response.send_message(f"Map '{map_name}' removed from category '{category_name}'.", ephemeral=True)
            else:
                await ctx.response.send_message(f"Map '{map_name}' does not exist in category '{category_name}'.", ephemeral=True)
        else:
            await ctx.response.send_message(f"Category '{category_name}' does not exist.", ephemeral=True)
    else:
        await ctx.response.send_message("You do not have the required permissions to perform this action.", ephemeral=True)
    ...
    if "Admin" in [role.name for role in ctx.user.roles]:
        try:
            ...
            if not init.MAPS.sections():
                await ctx.response.send_message("No sections found in the file.", ephemeral=True)
                return
            reply = "```"
            for section in init.MAPS.sections():
                ...
                reply += "\n"
            reply += "```"
            await ctx.response.send_message(f"Current map config:\n{reply}", ephemeral=True)
        except FileNotFoundError:
            await ctx.response.send_message("The configuration file does not exist.", ephemeral=True)
        except Exception as e:
            await ctx.response.send_message(f"An error occurred: {e}", ephemeral=True)
    else:
        await ctx.response.send_message("You do not have the required permissions to perform this action.", ephemeral=True)

@init.bot.tree.command(name="regeneratemapimage", description="Regenerate the image for the map pool")
async def regeneratemapimage(ctx: discord.Interaction):
    if "Admin" in [role.name for role in ctx.user.roles]:
        create_aggregate_image(parse_file(init.config_file), cache_directory, 'bot/mapsimage.jpg')
