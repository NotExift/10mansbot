...
class ConfigSelect(discord.ui.Select):
    def __init__(self, config_files):
        options = [
            discord.SelectOption(label=f"{filename}", description=f"Load {filename}", value=filename)
            for filename in config_files
        ]
        super().__init__(placeholder='Choose a configuration file...', min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        init.config_file = os.path.join(CONFIG_DIRECTORY, self.values[0])
        init.MAPS.clear()
        init.MAPS.read(init.config_file)
        await interaction.response.send_message(f"Loaded configuration from {init.config_file}!", ephemeral=True)
