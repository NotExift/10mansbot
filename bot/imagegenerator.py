...
import os
import textwrap
from imagegetter import ensure_cache_directory_exists, is_cached, cache_thumbnail, fetch_thumbnail_url
...
def create_aggregate_image(sections, cache_dir, output_path):
    ...
    total_height = (max_maps_in_section * (thumb_max_height + spacing)) + section_header_height + 60
    ...
    draw.text(((total_width - title_width) // 2, 10), title, font=title_font, fill=(0, 0, 0))
    ...
    for section, items in sections.items():
        ...
        for name, id_ in items:
            ...
            if os.path.exists(cached_image_path):
                ...
                img = Image.new('RGB', (thumb_max_width, thumb_max_height), color=(192, 192, 192))

            composite_image.paste(img, (x_offset, y_offset))
            wrapped_text = textwrap.fill(name, width=16)
            draw.text((x_offset + thumb_max_width - 50, y_offset), wrapped_text, font=label_font, fill=(0, 0, 0))
